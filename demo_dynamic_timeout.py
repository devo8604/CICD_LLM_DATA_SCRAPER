...
def demonstrate_dynamic_timeouts():
    ...
    for size_name, size_bytes in test_sizes:
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            ...
            try:
                calculated_timeout = calculate_dynamic_timeout(temp_file.name, base_timeout=base_timeout, min_timeout=min_timeout, max_timeout=max_timeout)

                print(f"File size: {size_name:6s} -> Timeout: {calculated_timeout:4d}s (factor: {calculated_timeout / base_timeout:.2f}x)")
            finally:
