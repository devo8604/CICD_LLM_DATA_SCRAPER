...
from src.pipeline.export_service import ExportService  # noqa: E402
from src.pipeline.exporters import DataExporter  # noqa: E402
...
def test_llm_client_requires_config():
    ...
    try:
        LLMClient("http://localhost", "test-model", 3, 1)
    ...
    try:
        config = AppConfig()
        LLMClient("http://localhost", "test-model", 3, 1, config=config)
