...
class TestBatchProcessingService:
    ...
    @pytest.mark.asyncio
    async def test_process_files_batch_single_file(self):
        ...
        self.file_processing_service.process_single_file = AsyncMock(return_value=(True, 2))
        
        results = await self.service.process_files_batch(
            files=files,
            repo_name=repo_name,
            semaphore=semaphore,
            batch_num=1,
    ...
    @pytest.mark.asyncio
    async def test_process_files_batch_multiple_files(self):
        ...
        async def mock_process_single_file(file_path, repo_name, pbar=None):
        ...
        self.file_processing_service.process_single_file = AsyncMock(side_effect=mock_process_single_file)
    ...
    @pytest.mark.asyncio
    async def test_process_files_batch_empty_list(self):
    ...
    @pytest.mark.asyncio
    async def test_process_files_batch_exception_handling(self):
        ...
        self.file_processing_service.process_single_file = AsyncMock(side_effect=Exception("Processing error"))
