...
class TestCLIPrepareCommand:
    ...
    def test_prepare_command_with_all_options(self):
        ...
        with patch.object(
            sys,
            "argv",
            ["prog", "prepare", "--max-tokens", "2000", "--temperature", "0.5"],
        ):
            args = parse_arguments()
...
class TestCLIExportCommand:
    ...
    def test_export_command_csv(self):
        ...
        with patch.object(
            sys,
            "argv",
            ["prog", "export", "--template", "csv", "--output-file", "output.csv"],
        ):
            args = parse_arguments()
    ...
    def test_export_command_llama3(self):
        ...
        with patch.object(
            sys,
            "argv",
            ["prog", "export", "--template", "llama3", "--output-file", "output.txt"],
        ):
            args = parse_arguments()
...
class TestCLIMLXCommands:
    ...
    def test_mlx_remove_command(self):
        ...
        with patch.object(sys, "argv", ["prog", "mlx", "remove", "some-model-name"]):
            args = parse_arguments()
            assert args.command == "mlx"
    ...
    def test_mlx_info_command(self):
        ...
        with patch.object(sys, "argv", ["prog", "mlx", "info", "some-model-name"]):
            args = parse_arguments()
            assert args.command == "mlx"
